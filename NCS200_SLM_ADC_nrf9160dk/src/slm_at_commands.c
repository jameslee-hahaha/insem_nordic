...
#endif
#if defined(CONFIG_SLM_ADC)
#include "slm_at_adc.h"
#endif
...
int slm_at_init(void)
{
	...
	err = slm_at_adc_init();
	if (err) {
		LOG_ERR("ADC could not be initialized: %d", err);
		return -EFAULT;
	}
#endif
	return err;
...
#endif
#if defined(CONFIG_SLM_ADC)
	err = slm_at_adc_uninit();
	if (err) {
		LOG_ERR("ADC could not be uninit: %d", err);
	}
#endif
}
